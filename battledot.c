...
struct PlayerNode *front = NULL;
struct PlayerNode *current = NULL;
...
*/
int checkEmpty() { return front == NULL; }

/*
Description:
Inserts a player to the front of the player list and inserts the coordinates of
their ship into a Player class.

Arguments:
        char moniker[4]		: Three letter name of player + terminating
character int x_ship			: x coordinate of player ship int y_ship
: y coordinate of player ship

Returns:
        none
*/
void insertPlayer(char moniker[4], int x_ship, int y_ship) {
  struct PlayerNode *newplayer =
      (struct PlayerNode *)malloc(sizeof(struct PlayerNode));
...
/*
Description:
Removes a player from the player list and appropriately pairs up their
neighbours. There are different cases of removal like when the loser is at the
front of the list that require a different approach. This is why there are
